            ...
            {years.map((y) => (
              <div key={y._id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{new Date(y.startDate).toLocaleDateString()} – {new Date(y.endDate).toLocaleDateString()}</p>
                </div>
                {y.isCurrent && <Badge variant="success">Current</Badge>}
              </div>
            ))}
            {years.length === 0 && <div className="px-5 py-8 text-center text-sm text-gray-400">No academic years yet.</div>}
          </div>
        </div>
      )}
            ...
            {classes.map((c) => (
              <div key={c._id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{CLASS_LEVELS.find(l => l.value === c.level)?.label}</p>
                </div>
                <button onClick={() => deleteClass.mutate(c._id)} className="text-xs text-red-500 hover:text-red-700">Remove</button>
              </div>
            ))}
            {classes.length === 0 && <div className="px-5 py-8 text-center text-sm text-gray-400">No classes yet.</div>}
          </div>
        </div>
      )}
      ...
      {tab === 'sections' && (
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
            ...
            {sections.length === 0 && <div className="px-5 py-8 text-center text-sm text-gray-400">No sections yet.</div>}
          </div>
        </div>
      )}
      ...
      {tab === 'subjects' && (
        <div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
            ...
            {subjects.length === 0 && <div className="px-5 py-8 text-center text-sm text-gray-400">No subjects yet.</div>}
          </div>
        </div>
      )}
...
function YearForm({ onSubmit, loading }: { onSubmit: (d: Partial<AcademicYear>) => void; loading: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      ...
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isCurrent} onChange={e => setForm(f => ({ ...f, isCurrent: e.target.checked }))} /> Set as current year</label>
...
function SubjectForm({ onSubmit, loading }: { onSubmit: (d: Partial<SubjectDoc>) => void; loading: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      ...
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isElective} onChange={e => setForm(f => ({ ...f, isElective: e.target.checked }))} /> Elective subject</label>
