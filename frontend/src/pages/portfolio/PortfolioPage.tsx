...
const COLOR_MAP = {
  blue:   { bg: 'bg-blue-50',   icon: 'bg-blue-600',   badge: 'bg-blue-100 text-blue-700',   ring: 'ring-blue-100' },
  red:    { bg: 'bg-red-50',    icon: 'bg-red-500',    badge: 'bg-red-100 text-red-600',     ring: 'ring-red-100' },
  violet: { bg: 'bg-violet-50', icon: 'bg-violet-600', badge: 'bg-violet-100 text-violet-700', ring: 'ring-violet-100' },
};
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans antialiased">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <a
            href={productUrl('edu')}
            className="hidden md:inline-flex px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-sm font-bold transition-colors shadow shadow-amber-500/20"
          >
            View EduStack PK →
          </a>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 flex flex-col gap-3 text-sm font-medium text-gray-600">
            ...
            <a href={productUrl('edu')} className="mt-1 px-4 py-2.5 rounded-xl bg-amber-500 text-white text-center font-bold">
              View EduStack PK →
            </a>
          </div>
        )}
      ...
      <section className="relative overflow-hidden bg-gray-950 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-28 lg:pt-32 lg:pb-36 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-extrabold tracking-tight leading-none mb-6">
            We build{' '}
            <span className="text-amber-400">ERP platforms</span>
            <br />for Pakistan.
          </h1>
            ...
            {[
              {
                ...
                desc: 'Every product is designed around Pakistani workflows, payment methods (JazzCash, EasyPaisa), and regulatory requirements.',
              },
              {
                icon: '☁️',
                title: 'Cloud-Native',
                desc: 'Multi-tenant SaaS architecture. Each organisation gets an isolated subdomain and data environment. No shared databases.',
              },
              {
                icon: '📱',
                title: 'Mobile-Ready',
                desc: 'Every platform ships with a Flutter mobile app for Android and iOS, so staff and students can work from anywhere.',
              },
            ].map((item) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-5">
          ...
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm">
            {PRODUCTS.filter(p => p.href).map(p => (
              <a key={p.id} href={p.href!} className="hover:text-white transition-colors">{p.name}</a>
            ))}
